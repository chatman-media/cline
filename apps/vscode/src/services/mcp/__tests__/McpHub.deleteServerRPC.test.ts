...
/**
 * Regression tests for McpHub.deleteServerRPC() — CLINE-2097.
 *
 * Deleting one MCP server emptied the entire list on Windows: the settings
 * file watcher raced the plugin's own write, read a transient/empty file, and
 * updateServerConnections({}) tore down every connection. The fix sets
 * isUpdatingClineSettings around the write so the watcher skips its own event.
 *
 * These tests bypass the constructor's filesystem watcher side effects via
 * Object.create(McpHub.prototype) and inject only the state deleteServerRPC
 * touches, matching the existing McpHub.callTool.test.ts harness.
 */
